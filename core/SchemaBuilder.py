...
def schema_reader(filepath: str = SCHEMA_PATH) -> list:
    if not filepath:
        log_error("SCHEMA environment variable is not set")
        return []
...
def remove_schema_table(
    ...
    if not removed:
        if interactive:
            print(f"Table '{table_name}' was not found in schema.")
        return {
            "ok": True,
            "action": "not_found",
            "table": table_name
        }
    write_result = schema_builder(new_schema, filepath)
    if "error" in write_result:
        return {
            "error": write_result["error"],
            "table": table_name
    ...
    if interactive:
        print(f"Schema for table '{table_name}' removed.")
    return {
        "ok": True,
        "action": "removed",
        "table": table_name
